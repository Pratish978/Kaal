...
const ChatInterface: React.FC<ChatInterfaceProps> = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { 
      role: 'assistant', 
      text: "I hear you. It sounds like you're feeling anxious right now. You don't have to go through this alone." 
    }
  ]);
  ...
  // Persistent Session ID for the API call context
  const sessionIdRef = useRef(`sess_${Date.now()}_${Math.random().toString(36).substring(7)}`);
  ...
  const handleSendMessage = async () => {
    ...
    const newUserMessages: Message[] = [...messages, { role: 'user', text: userMsg }];
    ...
    try {
      ...
      if (data.reply) {
        setMessages([...newUserMessages, { role: 'assistant', text: data.reply }]);
      }
    } catch (error) {
      console.error("Connection Error:", error);
      setMessages([
        ...newUserMessages, 
        { role: 'assistant', text: "Sorry, I'm having trouble connecting to Kaal right now." }
      ]);
    } finally {
      setIsLoading(false);
    }
      ...
      <div className={`bg-[#F6F2ED] w-full max-w-3xl rounded-[40px] px-8 py-10 flex flex-col shadow-sm h-fit max-h-[82vh] mt-4 transition-all duration-1000 ease-out no-scrollbar ${
        ...
        <div className="w-full overflow-y-auto mb-6 space-y-6 no-scrollbar flex flex-col">
          {messages.map((msg, idx) => (
            <div 
              key={idx} 
              className={`flex w-full animate-in fade-in slide-in-from-top-2 duration-500 ${
            ...
            >
              ...
              >
                <h2 className="text-base font-bold text-[#3D3D3D] mb-1 leading-tight">
                  {msg.role === 'assistant' ? (idx === 0 ? "I hear you." : "Kaal AI") : "You"}
                </h2>
          ...
          <div className="relative">
            ...
            <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-5">
              ...
              <button 
                ...
                disabled={isLoading} 
                className={`${
                  isLoading ? 'text-gray-200 cursor-not-allowed' : 'text-[#3D3D3D] hover:text-black'
                } transition-colors`}
              >
